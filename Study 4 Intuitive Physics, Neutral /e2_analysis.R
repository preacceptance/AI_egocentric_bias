...
d$displayCond <- factor(d$displayCond, levels = c("Absent", "Adjacent", "Present", "Baseline"),
                        labels = c("scene-absent", "scene-adjacent", "scene-annotated", "scene-only"))
d$agentCond <- factor(d$agentCond, levels = c("Self", "AI"))
d$borderSize <- ifelse(d$displayCond == "annotations-only", 1.2, 0.4)
...
plotter <- function(data, value_column, input_title) {
  p <- ggplot(data, aes(x = displayCond, y = get(value_column), fill = agentCond)) +
    ...
    geom_bar(stat = "summary", fun = "mean", position = position_dodge(width = 0.9),aes(size = borderSize), color = "black")+
    geom_errorbar(stat = "summary", fun.data = "mean_cl_boot", 
                  position = position_dodge(width = 0.9), width = 0.2) +
    
    # **Add line segments per agent condition**
    stat_summary(aes(group = agentCond, color = agentCond),
                 fun = mean, geom = "line", linewidth = 0.9, position = position_dodge(width = 0.9)) +
    scale_fill_manual(values = c("#eedfdb", "#e2eefd"), labels = c("Self", "AI")) +
    
    # Ensure matching line colors
    scale_color_manual(values = c("Self" = "#8f2d14", "AI" = "#4294f7"), guide = "none")+
    scale_size_identity()+
...
#figure <- ggarrange(p2, nrow = 1, ncol = 2)
...
ggsave("figure7.pdf", p2, width = 4, height = 5, dpi = 300)
